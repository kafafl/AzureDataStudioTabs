 ...
 AS 
  ...
  BEGIN
    ...
    DECLARE @Ticker AS VARCHAR(255)
      ...
      IF @PrevDate IS NULL
        BEGIN
          --SELECT TOP 1 @PrevDate = CAST(epd.AsOfDate AS DATE) FROM dbo.EnfPositionDetails epd WHERE 1 = 1  AND epd.AsOfDate < @AsOfDate ORDER BY epd.AsOfDate DESC
          SELECT @PrevDate = DATEADD(dd, -7, @AsOfDate)
        END
