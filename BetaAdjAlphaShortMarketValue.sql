...
/*
      ...
      WHERE sbv.BbgYellowKey = 'MSA1BIOH index'
      AND sbv.AsOfDate BETWEEN '08/15/2024' AND '09/10/2024'
      ORDER BY sbv.Bmkbeta DESC


      DECLARE @AsOfDate AS DATE = '10/1/2024'
      SELECT @AsOfDate,
             sbv.PortfolioName,
             sbv.Ticker,
             sbv.BbgYellowKey,
       ...
       WHERE sbv.AsOfDate BETWEEN DATEADD(D, -7, @AsOfDate) AND @AsOfDate
         AND sbv.BbgYellowKey = 'IMVT US Equity'
       GROUP BY sbv.PortfolioName,
             ...
             UPDATE sbv
                SET sbv.BmkBeta = 1.3
                FROM dbo.StatisticalBetaValues sbv
                WHERE sbv.Iid IN (9393, 9807, 9669)
...
/*   CLEAR CONTENTS OF DAILY TABLE  */
     ...
     INSERT INTO #tmpPortfolio(
            ...
            PosShort,
            Crncy)
       EXEC dbo.p_GetShortPortfolio @AsOfDate = @AsOfDate, @bIncludeOptions = 1, @bRetCrncy = 1
