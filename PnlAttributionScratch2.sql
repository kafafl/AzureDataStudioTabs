...
*/
   ...
   DECLARE @IncludeOptions AS BIT = 1
  ...
  /* ONLY USE MARKET DATES */
    IF NOT EXISTS(SELECT 1 FROM dbo.DateMaster dmx WHERE dmx.AsOfDate = @dtCurDate AND dmx.IsMktHoliday = 0 AND dmx.IsWeekday = 1)
      BEGIN
        SELECT TOP 1 @dtCurDate = dmx.AsOfDate FROM dbo.DateMaster dmx WHERE dmx.AsOfDate > @dtCurDate AND dmx.IsMktHoliday = 0 AND dmx.IsWeekday = 1 ORDER BY dmx.AsOfDate ASC
      END
       
      SELECT TOP 1 @dtPrevDate = dmx.AsOfDate FROM dbo.DateMaster dmx WHERE dmx.AsOfDate < @dtCurDate AND dmx.IsMktHoliday = 0 AND dmx.IsWeekday = 1 ORDER BY dmx.AsOfDate DESC
     ...
     INSERT INTO #tmpPositions(
            ...
            InstrTypeUnder,
            PrevBusDayNMV)
       EXEC dbo.p_GetEnfPositionData @AsOfDate = @dtCurDate
...
/*
     UPDATE tpx
        SET tpx.BBYellowKey = 'TALS US Equity' 
       FROM #tmpPositions tpx
      WHERE tpx.BBYellowKey = 'TRML US Equity'
        AND tpx.AsOfDate = @dtCurDate
*/

-- SELECT * FROM #tmpPositions tpx
 ...
 /*  Equities query for direct shares  */
     ...
     SELECT tpx.AsOfDate,
      ...
      GROUP BY tpx.AsOfDate,
            tpx.BBYellowKey,
            tpx.CcyOne
     HAVING SUM(COALESCE(tpx.Quantity, 0)) != 0
...
/*   SET HEDGE VALUES XBI US Equity   */
     ...
     UPDATE taa
        SET taa.MtmDtdPnl = COALESCE(taa.PrevMarketValue, 0) * COALESCE(taa.CurrReturn, 0)
       FROM #tmpAttribution taa
      WHERE taa.AsOfDate = @dtCurDate 
...
*/
    ...
    SELECT taa.AsOfDate,
           ...
           taa.HedgeName,
           taa.HedgeMarketValue,
           taa.HedgePerformance,
           taa.HedgeDtdPnl,
           taa.HedgedPnlUsd AS HedgedPnl
      FROM #tmpAttribution taa 
