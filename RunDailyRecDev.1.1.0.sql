 ...
 /*
  ...
  Example:  EXEC dbo.p_RunDailyPositionRec @AsOfDate = '03/01/2024', @RstOutput = 2
            ...
            EXEC dbo.p_RunDailyPositionRec @AsOfDate = '11/29/2023', @RstOutput = 1
...
/*  LOAD ENFUSION POSITION TEMP TABLE          */
    ...
    UPDATE tpd
       SET tpd.LongShort = CASE WHEN tpd.LongShort = 'Long' THEN 'L' WHEN tpd.LongShort = 'Short' THEN 'S' END
      FROM #tmpEnfPositionDetails tpd
      
    UPDATE tpd
       SET tpd.BBYellowKey = REPLACE(tpd.BBYellowKey, ' ELEC ', ' ')
      FROM #tmpEnfPositionDetails tpd
...
/*  LOAD ADMIN POSITION TEMP TABLE          */
    ...
    UPDATE tpd
       SET tpd.BbgCode = REPLACE(tpd.BbgCode, ' ELEC ', ' ')
      FROM #tmpAdminPositions tpd
...
/*
SELECT TOP 10 * FROM #tmpRecResults trr
WHERE CHARINDEX('AMAM', trr.AdmTicker) != 0
SELECT TOP 100 * FROM #tmpAdminPositions tap
WHERE CHARINDEX('AMAM', tap.BbgCode) != 0
RETURN
*/
...
/*
DECLARE @CheckValue AS VARCHAR(255) = 'PNT US 01/19/24 P12.5 Equity'
SELECT TOP 10 * FROM #tmpRecResults trr WHERE trr.AdmTicker = @CheckValue OR trr.Ticker = @CheckValue
SELECT TOP 10 * FROM #tmpAdminPositions apd WHERE apd.BbgCode = @CheckValue
SELECT TOP 10 * FROM #tmpEnfPositionDetails epd WHERE epd.BBYellowKey = @CheckValue
RETURN
*/
...
SELECT * FROM #tmpRecResults

SELECT * FROM #tmpAdminPositions tap WHERE tap.bIsUsed = 0
SELECT * FROM #tmpEnfPositionDetails epd WHERE epd.bIsUsed = 0
RETURN
...
/*
SELECT * FROM #tmpAdminPositions apd WHERE CHARINDEX('ABIVAX', apd.SecName) != 0
SELECT * FROM #tmpEnfPositionDetails apd WHERE CHARINDEX('ABIVAX', apd.InstDescr) != 0
SELECT * FROM #tmpRecResults apd WHERE CHARINDEX('ABIVAX', apd.AdmSecName) != 0
...
/*  SET USAGE  */
    UPDATE epd
       SET epd.bIsUsed = 1,
           epd.UsageNote = 'Enfusion UNMAPPED positions'
      FROM #tmpEnfPositionDetails epd 
     WHERE epd.AsOfDate = epd.AsOfDate
       AND epd.bIsUsed = 0
...
/*
SELECT * FROM #tmpRecResultsNonSec WHERE Strategy = 'CLOSED POSITIONS'
RETURN
...
SELECT * FROM #tmpAdminPositions apd WHERE apd.BbgCode = 'UXV3 Index'

SELECT * FROM #tmpEnfPositionDetails apd WHERE apd.bIsUsed = 0
...
/*
SELECT * FROM #tmpAdminPositions trr WHERE trr.bIsUsed = 0
SELECT * FROM #tmpEnfPositionDetails epd WHERE epd.bIsUsed = 0
*/
...
/*
            ...
            SELECT RTRIM(LTRIM(epd.CcyOne)) AS AggTicker,
             ...
             WHERE epd.AsOfDate = epd.AsOfDate
               AND epd.InstrType IN ('Cash', 'FX Forward') 
               AND epd.Account IN ('MS FX')
             GROUP BY RTRIM(LTRIM(epd.CcyOne)) 



SELECT *       FROM #tmpRecResults trr
WHERE trr.bUpdated = 0
       AND trr.Strategy = 'CURRENCY FORWARDS, CASH and SPOTS' 
 
SELECT *       
  FROM #tmpRecResults trr
      JOIN (SELECT RTRIM(LTRIM(REPLACE(REPLACE(RTRIM(LTRIM(epd.InstDescr)), 'Settled Cash -', ''), 'Un', ''))) AS AggTicker,
                   SUM(epd.Quantity) AS Quantity,
                   SUM(epd.DlyPnlUsd) AS DlyPnlUsd,
                   SUM(epd.MtdPnlUsd) AS MtdPnlUsd,
                   SUM(epd.YtdPnlUsd) AS YtdPnlUsd
             ...
             WHERE epd.AsOfDate = epd.AsOfDate
               AND epd.InstrType IN ('Cash', 'FX Forward') 
               AND epd.Account NOT IN ('Non-Trading','MS FX')
             GROUP BY REPLACE(REPLACE(RTRIM(LTRIM(epd.InstDescr)), 'Settled Cash -', ''), 'Un', '')) epd 
        ON trr.AdmSecName  = epd.AggTicker
     WHERE trr.bUpdated = 0
       AND trr.Strategy = 'CURRENCY FORWARDS, CASH and SPOTS' 
       RETURN
...
/*  ADD ENFUSION MAPPING TO CASH FORWARD AND SPOTS 
    UPDATE trr
       SET trr.Ticker = epd.AggTicker,
           trr.SecName = epd.AggTicker,
           trr.EnfQuantity = epd.Quantity,
           trr.EnfDtdPnlUsd = epd.DlyPnlUsd,
           trr.EnfMtdPnlUsd = epd.MtdPnlUsd,
           trr.EnfYtdPnlUsd = epd.YtdPnlUsd,           
           trr.bUpdated = 1,
           trr.iSort = 5,
           trr.UpdateRule = 'Cash Mapping'
      FROM #tmpRecResultsNonSec trr
      JOIN (SELECT RTRIM(LTRIM(epd.CcyOne)) AS AggTicker,
                   SUM(epd.Quantity) AS Quantity,
                   SUM(epd.DlyPnlUsd) AS DlyPnlUsd,
                   SUM(epd.MtdPnlUsd) AS MtdPnlUsd,
                   SUM(epd.YtdPnlUsd) AS YtdPnlUsd
             ...
             WHERE epd.AsOfDate = epd.AsOfDate
               AND epd.InstrType IN ('Cash', 'FX Forward') 
               AND epd.Account IN ('MS FX')
             GROUP BY RTRIM(LTRIM(epd.CcyOne))) epd 
        ON trr.AdmSecName = epd.AggTicker
     WHERE trr.bUpdated = 0
       AND trr.Strategy = 'CURRENCY FORWARDS, CASH and SPOTS' 
 */